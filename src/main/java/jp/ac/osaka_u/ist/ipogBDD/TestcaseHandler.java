...
//	byte[] value; // 0..level-1, or <0 (wildcard)
	int[] value; // 0..level-1, or <0 (wildcard)
	...
	// TODO Outputer.java に移動-> つかわないようにした
	...
	// TODO Outputer.java に移動	w
	// id2は，もとの順のパラメータが，サイズ順に並び替えたとき何番目だったかを指す
	// これは，`testcase`のパラメータ順が，サイズ順に並び替えたときのそれだからである．
	void print(BufferedWriter writer, PList paralist)
			throws IOException {
		boolean firstIteration = true;
		for (Parameter P: paralist) {
			int i = P.id2;
			writer.write((firstIteration ? "" : ",")
					+
					(value[i] >= 0 ? P.value_name.get(value[i]) : 
						(Main.randstar == Main.Randstar.ON ? "*" : "")
						));
